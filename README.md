...
When a photo is imported, Google Cloud Vision analyzes the image and generates descriptive labels. Those labels are then passed to Claude (Anthropic) which decides whether the image fits an existing bucket or whether a new bucket should be created. Bucket names are generated by Claude based on the content of the images inside them. As more photos are uploaded, the AI continuously re-evaluates and refines the bucket organization.
